...
int main(int argc, char **argv)
{
    ...
    Tensor<double> A = read(fs::path(params.input) / "A.ttx", Format({Dense, Sparse, Sparse, Sparse, Sparse}), true);
    ...
    IndexVar i, j, k, l, m, p;

    C_T(j, i) += A(i, k, l, m, p) * B_T(j, m) * B_T(j, l) * B_T(j, k) * B_T(j, p);
